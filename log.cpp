...
int log::init(const char* path, const char * prefix, 
			int max_size /*= 8<<20*/,  bool immediate_flush /*= true*/){

	const char * suffix = "-19790101-240000.log";
	int path_len = strlen(path);
	if(path_len + strlen(prefix) + strlen(suffix) >= max_path_len){
		return -1;
	}
	...
	if(path[path_len -1] != '/'){
		sprintf(m_pathname, "%s/%s", path, prefix);
	}
	else{
		sprintf(m_pathname, "%s%s", path, prefix);
	}
	...
	m_brun = true;
	if(create_thread(NULL, log_task, "log_task", this) < 0){
